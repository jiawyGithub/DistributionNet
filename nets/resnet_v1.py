    ...
    with slim.arg_scope([slim.conv2d, bottleneck,
      ...
      with slim.arg_scope([resnet_utils.extra_fc], loss_collection=tf.GraphKeys.REGULARIZATION_LOSSES):
          with slim.arg_scope([slim.batch_norm], is_training=is_training):
            ...
            end_points = slim.utils.convert_collection_to_dict(end_points_collection)
            # 关键代码
            # tf.variable_scope 1）变量共享；2）tensorboard画流程图进行可视化封装变量。
            with tf.variable_scope('Distributions'):
                ...
                tfd = tf.contrib.distributions
                #MultivariateNormalDiagWithSoftplusScale 多变量正态分布 说明文档https://tensorflow.google.cn/versions/r1.15/api_docs/python/tf/contrib/distributions/MultivariateNormalDiagWithSoftplusScale
                # Y = scale @ X + loc.   loc 均值 scale 方差
                sample_dist = tfd.MultivariateNormalDiagWithSoftplusScale(loc=end_points['PreLogits_mean'],
                                                         scale_diag=end_points['PreLogits_sig'])

                end_points['sample_dist'] = sample_dist
                end_points['sample_dist_samples'] = sample_dist.sample(100) # 生成100个样本 然而并没有用
                end_points['sample_dist_covariance'] = sample_dist.stddev() # 标准差
